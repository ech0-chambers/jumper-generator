<script>
  import { BezierCubic, Coordinate, X, Y, Line } from '$lib/geometry';
    ...
    let stitches = Math.round(measurements.st_per_inch * measurements.W / 2) * 2; // Ensure an even number of stitches
    let ribbing_rows = Math.round(measurements.row_per_inch_rib * measurements.H_rib);

    let B_arm = new BezierCubic(
        X(0),
        X(-(measurements.W - measurements.W_s) / 4),
        new Coordinate(-(measurements.W - measurements.W_s) / 2, measurements.H_cap / 4),
        new Coordinate(-(measurements.W - measurements.W_s) / 2, measurements.H_cap / 2),
    );
    let H_arm = measurements.A_c / 2 - B_arm.length;
    let C_neck;
    if (measurements.neckline == "v-neck") {
        C_neck = new Line(Coordinate.origin(), new Coordinate(measurements.W_n / 2, measurements.H_n));
    ...
    let armhole_start = Math.round(measurements.row_per_inch * (measurements.H - measurements.H_rib - H_arm - measurements.H_cap / 2));
    let neckline_start = Math.round(measurements.row_per_inch * (measurements.H - measurements.H_rib - measurements.H_n));
    let arm_first = armhole_start < neckline_start;
    let straight_rows = Math.min(armhole_start, neckline_start);
    let armhole_changes = B_arm.scaleXY(measurements.st_per_inch, measurements.row_per_inch).to_changes();
    let neckline_changes = C_neck.scaleXY(measurements.st_per_inch, measurements.row_per_inch).to_changes();
    let armhole_end = armhole_start + armhole_changes[armhole_changes.length - 1].r;
    let neckline_end = neckline_start + neckline_changes[neckline_changes.length - 1].r;
    // add the start row to the changes
    armhole_changes = armhole_changes.map(change => ({ r: change.r + armhole_start, diff: change.diff }));
    neckline_changes = neckline_changes.map(change => ({ r: change.r + neckline_start, diff: change.diff }));
    ...
    if (overlap) {
        if (arm_first) {
            ...
            initial_changes = armhole_changes.filter(change => change.r < neckline_start);
            combined_changes = armhole_changes.filter(change => change.r >= neckline_start).map(change => ({
                r: change.r,
            ...
            neckline_changes.forEach(change => {
                ...
                if (existing) {
                    ...
                    combined_changes.push({ r: change.r, armhole_diff: 0, neckline_diff: change.diff });
                }
            });
            ...
            initial_changes = neckline_changes.filter(change => change.r < armhole_start);
            combined_changes = neckline_changes.filter(change => change.r >= armhole_start).map(change => ({
                r: change.r,
                armhole_diff: 0,
            ...
            armhole_changes.forEach(change => {
                ...
                if (existing) {
                    ...
                    combined_changes.push({ r: change.r, armhole_diff: change.diff, neckline_diff: 0 });
                }
            });
            // sort combined_changes by row
            combined_changes.sort((a, b) => a.r - b.r);
...
<h2 class:mobile={appState.mobile}>Front</h2>

<div>
    <h3 class:mobile={appState.mobile}>Body</h3>
    <ol>
        <li>Cast on {stitches} stitches ({stitches / 2} left to {stitches / 2} right) for 1&#xD7;1 ribbing.</li>
  ...
  <Table headers={["Row Counter", "Both Edges"]}>
    ...
    {/each}
        ...
        <ol>
            <li>Bring half the stitches into hold, or take them off on waste yarn.</li>
        </ol>
        ...
        <ol start= "2">
            ...
            <li>Bring the held needles back into work (or re-hang from the waste yarn).</li>
